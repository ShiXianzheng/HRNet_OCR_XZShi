...
def main_worker(gpu, ngpus_per_node, args, config):
    ...
    class_weights = None
    # 1 / weights, single-polsar training
    class_weights = torch.FloatTensor([3.385, 21.8, 5.025,
                                       24.09, 17.24, 4.975,
                                       28.65, 14.71, 18.66]).cuda()   # 384.6,
    ...
    test_size = (config.TRAIN.IMAGE_SIZE[1], config.TRAIN.IMAGE_SIZE[0])
    ...
    for epoch in range(last_epoch, end_epoch):
        ...
        train(config, epoch, config.TRAIN.END_EPOCH, epoch_iters, config.TRAIN.LR,
              num_iters, train_loader, optimizer, model, writer_dict, criterion, args)

        # valid process
        mean_IoU, mean_OA, fw_iou, rank = validate(config, test_loader, model, writer_dict, criterion, args)

        # save process
        net_name = ['HRNet_', 'OCR_']
        ...
        if rank == 0:
            ...
            for i in range(config.MODEL.NUM_OUTPUTS):
                if mean_OA[i] > best_OA[i]:  # save every 4 epoch
                    sv_name = net_name[i] + 'epoch_' + str(epoch + 1) + '_fwiou_' + str(fw_iou[0])[:5] + '_OA_' + str(mean_OA[0])[:5] + '.pth'
                    torch.save(model.module.state_dict(), os.path.join(final_output_dir, sv_name))

                # save best_mIoU
                if mean_IoU[i] > best_mIoU[i]:
                    best_mIoU[i] = mean_IoU[i]
                    torch.save(model.module.state_dict(), os.path.join(final_output_dir, net_name[i] + 'best_mIoU.pth'))

                if mean_OA[i] > best_OA[i]:
                    best_OA[i] = mean_OA[i]
                    torch.save(model.module.state_dict(), os.path.join(final_output_dir, net_name[i] + 'best_OA.pth'))

                if fw_iou[i] > best_fw_iou[i]:
                    best_fw_iou[i] = fw_iou[i]
                    torch.save(model.module.state_dict(), os.path.join(final_output_dir, net_name[i] + 'best_fw_iou.pth'))

                msg = '{}, OA: {:.4f}, m_iou: {:.4f}, fw_iou: {:.4f}, Best_OA: {:.4f}, Best_fw_iou: {:.4f}'\
                    .format(net_name[i], mean_OA[i], mean_IoU[i], fw_iou[i], best_OA[i], best_fw_iou[i])

                logging.info(msg)
                # logging.info(IoU_array[i])
